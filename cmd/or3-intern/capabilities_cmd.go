...
func printCapabilitiesReport(w io.Writer, report controlplane.CapabilitiesReport) {
	...
	_, _ = fmt.Fprintf(w, "execution_model: %s\n", report.ExecutionModel)
	_, _ = fmt.Fprintf(w, "default_runner: %s\n", report.DefaultRunner)
	_, _ = fmt.Fprintf(w, "runner_permissions: mode=%s isolation=%s\n", report.RunnerMode, report.RunnerIsolation)
	_, _ = fmt.Fprintf(w, "service_terminal_available: %t\n", report.TerminalAvailable)
