...
Cron payloads can also use `kind: "runner_run"` to enqueue an external agent runner such as Codex, Claude, Gemini, or OpenCode. These jobs require `payload.agent_run.runner_id` and `payload.agent_run.task`; when `mode` and `isolation` are omitted they default to read-only review (`review` + `host_readonly`).
