...
func codexHomeEnv(cfg config.AgentCLIConfig) (map[string]string, error) {
...
func resolveCodexHomeLayout(cfg config.AgentCLIConfig) (codexHomeLayout, bool, error) {
	...
	if home == "" {
		return codexHomeLayout{}, false, fmt.Errorf("agentCLI.codexShadowHomePath requires agentCLI.codexHomePath")
	}
	...
	if sharedHome == shadowHome {
		return codexHomeLayout{}, false, fmt.Errorf("agentCLI.codexShadowHomePath must be different from agentCLI.codexHomePath")
	}
