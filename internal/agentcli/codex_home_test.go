...
func TestCodexHomeEnvMaterializesShadowHome(t *testing.T) {
	...
	env, err := codexHomeEnv(config.AgentCLIConfig{CodexHomePath: sharedHome, CodexShadowHomePath: shadowHome})
	if err != nil {
...
func TestCodexHomeEnvRejectsShadowConflicts(t *testing.T) {
	...
	_, err := codexHomeEnv(config.AgentCLIConfig{CodexHomePath: sharedHome, CodexShadowHomePath: shadowHome})
...
func TestCodexHomeEnvExpandsHome(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	env, err := codexHomeEnv(config.AgentCLIConfig{CodexHomePath: "~/.codex-test"})
	if err != nil {
		t.Fatalf("codexHomeEnv: %v", err)
	}
