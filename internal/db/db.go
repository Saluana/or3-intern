...
func (d *DB) migrate(ctx context.Context) error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS secure_connection_host_identity(
			host_id TEXT PRIMARY KEY,
			host_signing_public_key TEXT NOT NULL,
			host_noise_public_key TEXT NOT NULL,
		...
		`CREATE TABLE IF NOT EXISTS secure_connection_devices(
		...
		`CREATE INDEX IF NOT EXISTS secure_connection_sessions_host_status ON secure_connection_sessions(host_id, status, expires_at);`,
		`CREATE INDEX IF NOT EXISTS secure_connection_sessions_device ON secure_connection_sessions(device_id, status);`,
		`CREATE TABLE IF NOT EXISTS secure_connection_pairing_sessions(
			...
			relay_origin TEXT NOT NULL DEFAULT '',
			account_id TEXT NOT NULL DEFAULT '',
			created_at INTEGER NOT NULL,
			expires_at INTEGER NOT NULL,
		...
		`CREATE INDEX IF NOT EXISTS secure_connection_pairing_sessions_host_status ON secure_connection_pairing_sessions(host_id, status, expires_at);`,
		`CREATE INDEX IF NOT EXISTS secure_connection_pairing_sessions_expiry ON secure_connection_pairing_sessions(status, expires_at);`,
		...
		`CREATE INDEX IF NOT EXISTS relay_routes_host_status ON relay_routes(host_id_hash, status, expires_at);`,
		`CREATE TABLE IF NOT EXISTS relay_rendezvous(
			rendezvous_id TEXT PRIMARY KEY,
			account_id TEXT NOT NULL DEFAULT '',
			host_id_hash TEXT NOT NULL,
			secret_commitment TEXT NOT NULL,
		...
		`CREATE INDEX IF NOT EXISTS relay_rendezvous_host_status ON relay_rendezvous(host_id_hash, status, expires_at);`,
		...
		`CREATE INDEX IF NOT EXISTS idx_chat_session_meta_updated ON chat_session_meta(archived, updated_at DESC);`,
		`CREATE INDEX IF NOT EXISTS idx_chat_session_meta_runner ON chat_session_meta(runner_id, updated_at DESC);`,
		`CREATE INDEX IF NOT EXISTS idx_chat_session_meta_parent ON chat_session_meta(parent_session_key, created_at DESC);`,
		...
		`CREATE INDEX IF NOT EXISTS idx_runner_chat_sessions_app ON runner_chat_sessions(app_session_key, updated_at DESC);`,
		...
		`CREATE INDEX IF NOT EXISTS idx_runner_chat_turns_session ON runner_chat_turns(session_id, sequence DESC);`,
		`CREATE INDEX IF NOT EXISTS idx_runner_chat_turns_status ON runner_chat_turns(status, requested_at);`,
		// Enforce one active turn per runner_chat_session via partial unique index.
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_runner_chat_turns_active
			ON runner_chat_turns(session_id) WHERE status IN ('queued','running');`,
		...
		`CREATE INDEX IF NOT EXISTS idx_runner_chat_events_turn_seq ON runner_chat_events(turn_id, seq);`,
		`CREATE INDEX IF NOT EXISTS idx_runner_chat_events_job_seq ON runner_chat_events(job_id, seq);`,
	}
	for _, s := range stmts {
