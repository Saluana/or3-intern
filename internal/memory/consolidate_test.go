...
func TestBuildCanonicalPinnedText_DeduplicatesExistingItems(t *testing.T) {
	text := buildCanonicalPinnedText("- dark mode\n- golang", []string{"dark mode"}, []string{"golang", "sqlite"})
	if strings.Count(text, "dark mode") != 1 {
		t.Fatalf("expected dark mode once, got %q", text)
	}
	if strings.Count(text, "golang") != 1 {
		t.Fatalf("expected golang once, got %q", text)
	}
	if !strings.Contains(text, "sqlite") {
		t.Fatalf("expected sqlite appended, got %q", text)
	}
}
...
func TestConsolidator_RunOnce_UsesResolvedScopeForPinnedAndNotes(t *testing.T) {
	d := openConsolidateTestDB(t)
	ctx := context.Background()
	...
	for i := 0; i < 14; i++ {
		...
		if _, err := d.AppendMessage(ctx, "session-a", role, "linked memory message", nil); err != nil {
			t.Fatalf("AppendMessage: %v", err)
		}
	}
	prov, _ := buildConsolidationProvider(t,
		`{"summary":"linked summary","facts":["shared scope fact"],"preferences":["shared scope preference"],"goals":[],"procedures":[]}`,
		false)
	c := &Consolidator{
		DB:                 d,
		Provider:           prov,
	...
	didWork, err := c.RunOnce(ctx, "session-a", 5, RunMode{})
	if err != nil {
		t.Fatalf("RunOnce: %v", err)
	}
	if !didWork {
	...
	pinned, ok, err := d.GetPinnedValue(ctx, "scope-1", "long_term_memory")
	if err != nil {
		t.Fatalf("GetPinnedValue scope: %v", err)
	}
	if !ok || !strings.Contains(pinned, "shared scope preference") {
		t.Fatalf("expected pinned memory written to resolved scope, got ok=%v value=%q", ok, pinned)
	}
	results, err := d.SearchFTS(ctx, "scope-1", "shared scope fact", 5)
	if err != nil {
		t.Fatalf("SearchFTS scope: %v", err)
	}
	if len(results) == 0 {
