...
func TestChatAdapterSessionRefHelpersAndUnknownTypes(t *testing.T) {
	...
	if events := codex.NormalizeChatEvent(RunnerRunEvent{Type: "structured", Payload: json.RawMessage(`{"type":"unknown"}`)}); len(events) != 1 || events[0].Type != "runner_output" {
	...
	if events := opencode.NormalizeChatEvent(RunnerRunEvent{Type: "structured", Payload: json.RawMessage(`{"type":"unknown"}`)}); len(events) != 1 || events[0].Type != "runner_output" {
