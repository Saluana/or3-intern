...
// The callback receives typed RunnerRunEvent values with monotonic sequence numbers.
func (p *ProcessManager) Run(ctx context.Context, spec CommandSpec, onEvent func(RunnerRunEvent)) ProcessOutput {
	startedAt := time.Now()
...
func emitError(onEvent func(RunnerRunEvent), seq int64, msg string) {
	if onEvent == nil {
		return
	}
