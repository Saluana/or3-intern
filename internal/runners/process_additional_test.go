...
func TestProcessManagerBinaryNotFoundAndStderrOnly(t *testing.T) {
	...
	out := pm.Run(context.Background(), CommandSpec{Binary: "missing-runner", Env: []string{"PATH=" + t.TempDir()}}, func(e RunnerRunEvent) {
...
func TestProcessManagerDefaultsNilCallbackAndCancelledProcess(t *testing.T) {
	...
	cancel()
	var events []RunnerRunEvent
	out = pm.Run(ctx, CommandSpec{Binary: "cancel-me", Env: []string{"PATH=" + dir}}, func(e RunnerRunEvent) {
		events = append(events, e)
	})
	if out.ExitCode != -1 {
