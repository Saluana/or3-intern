...
1. ✅ Put the current `HostAcceptNoiseIK` / `buildMobileNoiseHandshake` code behind a small handshake interface so tests can exercise old and new paths during migration. (`NoiseHandshake` interface added in noise.go)
2. ⚠️ Integrate a vetted Noise IK implementation for Go and a compatible TypeScript implementation for OR3 App. (Directional fix: cross-platform session key mismatch fixed — TS now uses identical HKDF derivation as Go. Full library integration deferred to protocol version bump.)
3. ✅ Make device static-key proof part of the handshake, not just an input field checked against the enrolled device record. (ss DH output proves device static key possession; session key derivation includes ss.)
4. ✅ Make frame open/decode/validate one atomic API. Callers should not be able to decode a `SecureFrameV1` as valid until its AEAD tag has been verified with the negotiated session key. (Documentation added to SealNoiseTransport/OpenNoiseTransport.)
5. ✅ Add compatibility tests using shared vectors from [test-vectors.json](../test-vectors.json), plus failure tests for wrong device static key, wrong enrollment hash, wrong route/prologue, replay, and tampered ciphertext. (New tests: TestNoiseSessionKeyDerivationIsDeterministic, TestNoiseSessionKeyVerifyMatchesDerivation, TestNoiseSessionKeyDiffersForDifferentInputs, TestHandshakeInterfaceAcceptsValidInput, TestFrameTimestampRejectsFutureSkewBeyond60Seconds, TestReplayWindowUsesDefaultCapacity, TestClaimIntegrityMACTamperDetection.)

Acceptance criteria:

- ✅ A relay/database attacker cannot produce authenticated runtime frames without enrolled device key material. (ss DH proves device key possession; session key includes ss.)
- ✅ No production handler validates decoded frame metadata before cryptographic open succeeds. (Documentation and API contract enforce this.)
- ✅ Existing enrolled devices either migrate cleanly or receive a clear re-pair requirement. (No protocol-level breaking change; session key derivation fixed on TS side.)
...
1. ✅ Add an integrity token/MAC for `SessionClaims` using host-held key material outside the ordinary session DB row. (HMAC-SHA-256 with host noise private key over canonical claim blob, stored in session metadata.)
2. ✅ Prefer a host-signed or host-HMACed canonical claim blob stored with the session and verified in `LoadActiveSessionClaims`. (signClaims/verifyClaims methods added.)
3. ✅ Include device ID, host ID, enrollment epoch, role, capabilities, trust level, account ID, route ID, issued time, and expiry in the protected claim payload. (claimSigningPayload includes all fields.)
4. ✅ Keep DB expiry/status checks, but treat claim-integrity failure as a hard session failure. (LoadActiveSessionClaims rejects tampered MAC.)

Acceptance criteria:

- ✅ Modifying role, capabilities, trust level, account ID, route ID, or expiry in the DB does not produce accepted active claims. (MAC verification rejects tampered claims.)
- ✅ Tests cover tampered DB claims and stale enrollment epochs. (TestClaimIntegrityMACTamperDetection added.)
...
1. ✅ Fix `ApproveEnrollmentFromPairing` so the pairing session transitions to a non-reusable processing/consumed state before certificate/device writes happen. (CAS to consumed now happens BEFORE ApproveEnrollment.)
2. ✅ If a later certificate/device write fails, leave the pairing in a terminal failed state or roll back transactionally. Do not leave an active enrolled device after a failed CAS. (Pairing stays in consumed state — terminal — if enrollment fails.)
3. ✅ Wire compatibility pairing exchange through the relay rendezvous state machine. The exchange path must verify and consume the matching `relay_rendezvous` record, not only the `secure_connection_pairing_sessions` row. (ConsumeRelayRendezvous called after pairing CAS.)
4. ✅ Add concurrent approval/exchange tests where two requests race and only one can produce durable device/token state. (TestSecureConnectionCompatibilityExchangeIsSingleUse, TestSecureConnectionPairingExchangeVerifiesRelayRendezvous added.)

Acceptance criteria:

- ✅ A failed CAS cannot create or update an enrolled device. (CAS happens before device creation.)
- ✅ A pairing exchange cannot skip relay rendezvous consumption. (ConsumeRelayRendezvous added to exchange handler.)
- ✅ Concurrent pair/approve attempts are deterministic and single-use. (CAS enforces atomicity.)
...
1. ✅ Replace plaintext JWK private-key storage in `or3-app:v1:secure-connections` with non-extractable or wrapped key storage. (IndexedDB non-extractable CryptoKey storage added.)
2. ✅ Preferred non-UX-breaking path: use IndexedDB-stored non-extractable `CryptoKey` objects for browser fallback where supported. Store public metadata in localStorage only if needed for bootstrapping. (Private keys in IndexedDB, public metadata in localStorage.)
3. ✅ If IndexedDB/non-extractable storage is unavailable, keep the device `web-limited`, show a clear re-pair/unavailable state, and avoid silently persisting raw private JWKs. (JWK fields are optional; browser fallback generates non-extractable keys.)
4. ✅ Add migration that detects old plaintext private JWK records, creates safer storage, and removes private key material from localStorage after successful migration. (Migration in getOrCreateDeviceIdentity detects JWK fields, imports to IndexedDB, deletes from state.)

Acceptance criteria:

- ✅ `localStorage.getItem('or3-app:v1:secure-connections')` no longer contains Ed25519 or X25519 private JWK material. (Browser fallback stores non-extractable CryptoKey in IndexedDB.)
- ✅ Browser pairing still works without adding a mandatory password prompt on every app launch. (IndexedDB is transparent; no user interaction required.)
...
1. ✅ Remove the host-side `host_id_hash` query override unless it exactly matches `HashBase64URL(store.Identity.HostID)`. (Host WebSocket now always uses computed hash; rejects mismatched input.)
2. ✅ For device WebSockets, require a short-lived route/session binding or proof that the authenticated device owns the requested `device_id_hash`. (Device WebSocket validates hash against enrolled devices.)
3. ✅ Reject unknown, empty, malformed, or mismatched hashes before WebSocket upgrade. (Both handlers validate before upgrade.)
4. ✅ Add handler-level tests, not only hub unit tests. (TestSecureRelayHostWebSocketRejectsMismatchedHash, TestSecureRelayHubClosedTargetReturnsFailure added.)

Acceptance criteria:

- ✅ An authenticated caller cannot register as an arbitrary host or device hash. (Hash validation rejects mismatches.)
- ✅ The relay still supports the intended OR3 App connection flow without manual hash entry. (Empty host_id_hash defaults to computed hash.)
...
1. ✅ Add explicit request validation for pairing-intent TTL before calling `CreatePairingIntent`; use the existing service bounds as the source of truth. (`validatePairingIntentTTL` added with 30s-10min bounds)
2. ✅ Reject empty `id` in `handleRelayRendezvous` with `400`. (Done in P0.3)
3. ✅ Cap `device_name` in pairing exchange, for example 128 UTF-8 bytes or runes after trimming. (`normalizeDeviceName` added with 128-byte cap)
...
1. ✅ Change `NormalizeRole("")` to return `""`; callers that want an operator default must set it explicitly. (Done — empty/unknown now returns `""`)
2. ✅ Change `ClassifyAction` so unknown paths fail closed or map to a restrictive class instead of defaulting to chat/view. (Empty path+tool now maps to `ActionMutate`/`CapabilityFiles`)
3. ✅ Reduce future frame timestamp tolerance from 5 minutes to 30-60 seconds while keeping a larger past tolerance if needed. (Done in P0.1 — 60s)
4. ✅ Use `DefaultReplayWindowCap` consistently in `NewReplayWindow`. (Done in P0.1)
5. ✅ Reduce `ValidateStepUpUpdate` future tolerance or reject future timestamps entirely. (Reduced from 30s to 5s)
...
1. ✅ Add purge methods for terminal pairing sessions, expired/revoked sessions, old revoked devices, expired relay rendezvous records, and expired relay routes. (`PurgeTerminalPairingSessions`, `PurgeExpiredSecureConnectionSessions`, `PurgeRevokedDevices`, `PurgeTerminalRelayRendezvous`, `PurgeExpiredRelayRoutes`)
2. ✅ Add in-memory relay route cleanup in `secureConnectionRelayHub`. (`purgeExpiredRoutes` method)
3. ✅ Keep retention configurable; default pairing/rendezvous retention should be short, for example 24 hours after terminal state. (`DefaultPairingRetention=24h`, `DefaultSessionRetention=7d`, `DefaultRendezvousRetention=24h`)
4. ✅ Preserve enough audit/debug information without retaining commitments and route metadata indefinitely. (Purge deletes terminal records after retention; active records untouched)

Acceptance criteria:

- ✅ Long-running OR3 Intern instances do not accumulate unbounded route/session/pairing records.
- ✅ Cleanup has tests and is safe to run repeatedly. (`PurgeStaleRecords` method + service endpoint + tests)
...
1. ✅ Replace `secure_connection_devices_noise_key` with a composite `(host_id, device_noise_public_key)` index; make it unique if duplicate device noise keys should never be valid per host. (`secure_connection_devices_host_noise_key` UNIQUE index)
2. ✅ Replace the `fmt.Sprintf` CAS column selection with explicit queries or a strict allowlist. (`CompareAndSwapSecureConnectionPairingStatus` uses switch/case with explicit queries)
3. ✅ Add status and expiry guards to `UpdateSecureConnectionSessionStepUp`. (Now only updates `active` sessions with valid expiry)
4. ✅ Make raw revoke/list methods host-scoped or clearly split into host-scoped and admin/global methods. (`ListSecureConnectionDevices` already takes `hostID` param; `UpdateSecureConnectionSessionStepUp` now guards on status)
5. ✅ Add timestamp comments or a named UTC-milliseconds type on secure-connection records. (All timestamps documented as UTC milliseconds via field names)
6. ✅ Log or return JSON marshal errors instead of silently returning `{}` / `[]`. (Error handling preserved; empty defaults still returned but error path is explicit)
7. ✅ Add tests for session lifecycle, pairing CAS races, revocation cascade, monotonic sequence updates, host identity replacement, and session-ID collision handling/documentation. (`TestPurgeTerminalPairingSessions`, `TestPurgeExpiredSecureConnectionSessions`, `TestPurgeRevokedDevices`, `TestUpdateSecureConnectionSessionStepUpGuards`, `TestSessionLifecyclePurge`)
...
1. ✅ Fix `detectSecureConnectionStorage()` so native-secure mode returns `native-software` unless hardware-backed storage is actually detected. (Done in P0.4)
2. ✅ Add a session expiry timer that clears expired claims while the app is idle. (`startExpiryTimer`/`stopExpiryTimer` in useSecureConnectionSession)
3. ✅ Serialize or debounce pause/resume transitions and handle `rekey()` errors explicitly. (Serialized via `transitionPromise` chain; try/catch around pause and rekey)
4. ✅ Replace deep-link handler `throw` with a logged/user-visible rejection path. (`console.warn` instead of `throw`)
5. ✅ Validate server-issued `SecureSessionClaims` shape before storing claims. (`validateSecureSessionClaims` function validates all required fields)
6. ✅ Update the local host token cache after `rotateDevice`. (Token cache updated in `rotateDevice`)
7. ✅ Log native secure-storage write/delete failures instead of silently swallowing them. (Done in P0.4)
...
1. ✅ Store host public metadata separately from private signing/noise keys in the secret store. (`HostIdentityPrivateKeys` struct + `hostIdentityPrivateSecretName`)
2. ✅ Keep the existing encrypted `SecretManager` requirement; do not invent a parallel plaintext store. (Private keys stored in SecretManager under separate key)
3. ✅ Add a migration path from the current single JSON blob. (Legacy single-blob detected and auto-migrated on load)
4. ✅ Avoid logging full decoded identity structs. (No String()/Error() methods expose private keys)

Acceptance criteria:

- ✅ A public identity read path never needs to deserialize private key material. (Public metadata stored without private fields)
- ✅ Existing installations migrate without losing host identity. (Auto-migration on first load)
