...
- [ ] Add route contract fixtures from current `or3-app` usage.
...
- [ ] Replace manual service route dispatch with a route table.
...
- [ ] Finish universal service error contract.
...
- [ ] Add structured service error enum documentation.
...
- [ ] Split auth middleware into composable steps without changing behavior.
  - Suggested files:
...
- [ ] Add auth method selection tests.
...
- [ ] Make auth failure codes more specific.
...
- [ ] Extract config and readiness startup builder.
...
- [ ] Extract runtime and scheduler builder.
...
- [ ] Replace the large `main()` body with high-level orchestration.
...
- [ ] Create a shared screen interface.
  - Suggested interface:
...
- [ ] Add smoke tests for each configure section.
...
- [ ] Make `settings` the canonical config entrypoint in help text.
...
- [ ] Consolidate device command language around pairing.
...
- [ ] Add shared CLI/TUI/non-TTY formatters.
...
- [ ] Introduce endpoint-specific request structs.
...
- [ ] Add duplicate-field conflict warnings.
...
- [ ] Document request body conventions.
...
- [ ] Add shared channel access helper package.
...
- [ ] Migrate channels incrementally.
...
- [ ] Add shared channel error formatting.
...
- [ ] Prefer structured service codes everywhere.
...
- [ ] Add bootstrap warning UI coverage.
...
- [ ] Update `docs/api-reference.md` after route/error tests land.
- [ ] Update `docs/configuration-reference.md` after request parsing cleanup lands.
- [ ] Add a migration note for `.env`, compose, quarantined integrations, and legacy context mode.
