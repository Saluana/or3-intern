...
- [ ] [Req 3] Update `cmd/or3-intern/doctor.go` and startup paths so service mode, enabled channels, or enabled automation trigger strict validation in hosted profiles.
- [ ] [Req 3] Enforce required secret-store, audit, outbound network policy, and safe MCP HTTP posture in `internal/security/*` and config validation.
- [ ] [Req 3, 4] Add clear startup refusal messages that tell operators when to move risky execution to `or3-sandbox` instead of broadening local permissions.
